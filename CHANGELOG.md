...
## v0.6.12 · 2026-08-25 · 建 .Core/OVERVIEW.md 项目全局一览(MATH-012)

**Agent 启动 30 秒建认知 · 解决"读 4 文件拼全貌"问题**

### 改动

- **新建 `.Core/OVERVIEW.md`** · 4208 字节 · 7 节
  - 1. 一句话定义 · 2. 当前状态(版本/阶段/进度/远端) · 3. 3 文件职责 · 4. 下一步 · 5. 关键链接导航 · 6. 关键决策(最近 5 条) · 7. Agent 操作入口
  - 高频读入口:agent 启动第一件事,30 秒读完即知项目全貌
- **AGENTS.md 第 2 段自动加 `.Core/` 目录**(按"架构自动同步规则",v0.6.7 起永久规则) — 增量改,不重写
- **cron `math-advisor-daily-wake` prompt** 加 0.5 步"读 .Core/OVERVIEW.md" 在 0 步(读 PLAN.md 之前),作为快速认知入口

### 文件职责最终划分

| 文件 | 职责 | 何时读 |
|---|---|---|
| `.Core/OVERVIEW.md` | 项目全局一览(短小,3-5 KB) | **启动第一件事** |
| `AGENTS.md` | 铁律 + 架构 + 能力库 | 启动第二件(规则遵守) |
| `PLAN.md` | 任务 + 路线图 + 产品开发计划 | 启动第三件(找任务) |
| `MEMORY.md`(agent 数据目录) | agent 临时工作笔记 | 启动第四件(看状态) |
| `CHANGELOG.md` | 版本审计(append-only) | 干活时 5 步归档要 append |
| `feedback_inbox.json` | 跨 agent 反馈 | 启动第五件(P0 催办) |

### 关闭

- ✅ MATH-012 · 建 .Core/OVERVIEW.md 项目全局一览(本条按新规则从 PLAN 删除)

---

## v0.6.11 · 2026-08-25 · PLAN.md 加"产品开发计划"段(MATH-011)

**融合 2 个上层产品 .md 做完整开发计划清单**

### 改动

- `PLAN.md` 加新段"## 产品开发计划(融合自 `数学顾问开发架构与计划.md` + `项目开发计划.md`)"
  - 8 节:项目代号+愿景 / 6 产品形态 / 5 大模块(知识图谱/题目生成/学习管理/AI Tutor/多端) / 技术栈 / Phase 0-4 迭代路线 / Phase 0 详细任务(含已完成✅) / Phase 1-4 简述 / 风险+下一步
  - 融合原则:取 `数学顾问开发架构与计划.md`(详版 432 行)为骨架 + `项目开发计划.md`(精版 313 行)的最新 Phase 0 状态为准
  - 标记关系:**MathematicsWeb = 数学顾问的 Web App 形态(Phase 1 产物)**
- `AGENTS.md` **不动** — 跟产品开发计划段不冲突(AGENTS.md 写项目级,产品开发计划写上层产品,视角独立)

### 关于 2 个 .md 源文件

- 保留原文件不删(用户没明确指示,git 可查历史)
- 内容已 100% 进入 PLAN.md"产品开发计划"段,2 个 .md 后续可作参考归档

### 关闭

- ✅ MATH-011 · 融合 2 个产品规划 .md 做开发计划清单(本条按新规则从 PLAN 删除)
