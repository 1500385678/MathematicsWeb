...
- [~] **MATH-009** · AGENTS 架构段 + docs/03 场景清单 同步实际项目
  - 描述: AGENTS.md 第 2 段目录树是 v0.1.0 的 6 场景结构,严重过期(v0.6.5 已 20 场景 + tools/ + docs/05 + knowledge_graph.json + server.py 等);docs/03 场景清单同样过期
  - 验收: (1) AGENTS.md 第 2 段目录树跟实际项目对齐 (2) docs/03_场景清单.md 补到 20 场景,每个含数学原理 + 简介 + 简写实现要点 (3) 按 5 步归档 + 新增"架构自动同步"规则一并文档化
  - 创建: 2026-08-25
  - 依赖: 无
  - 发现者: 用户指令
